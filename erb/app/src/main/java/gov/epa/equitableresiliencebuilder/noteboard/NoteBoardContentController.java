...
import gov.epa.equitableresiliencebuilder.engagement_action.ExternalFileUploaderController;
...
public class NoteBoardContentController implements Initializable {
	...
	protected HBox rankedItemsHBox;
	...
	@FXML
	Button nextButton, previousButton, saveBoardButton;
	@FXML
	VBox mainVBox, centerVBox, noteBoardItemVBox, dropDownVBox;
	@FXML
	HBox topLabelHBox, bottomLabelHBox, rightLabelVBox, leftLabelVBox;
	...
	private void indicatorChoiceBoxSelection() {
		...
		for (IndicatorCard indicatorCard : indicatorCards) {
			if (indicatorSystemSelected.contentEquals("All")
					|| indicatorCard.getSystem().contentEquals(indicatorSystemSelected)) {
	...
	private Parent loadNoteBoardRow(int numberOfColumns, boolean setRowDrag) {
		try {
			...
			if (setRowDrag)
				noteBoardRowController.turnOnRowDrag();
		...
		if (ids != null && ids.size() > 0) {
			try {
				...
				for (String id : ids) {
					...
					if (card != null)
						printWriter.println(card.getSystem() + "-" + card.getIndicator());
				}
				printWriter.close();
			} catch (FileNotFoundException e) {
	...
	private void nextButtonAction() {
		if (linearDataExists() && rankedMatchSaved()) {
			try {
				...
				NoteBoard_QuadrantRanking noteBoardContentController = new NoteBoard_QuadrantRanking(app,
						app.getEngagementActionController().getCurrentGoal(), erbContentItem, rowControllers,
						indicatorCards);
	...
	private void previousButtonAction() {
		if (previousButton.isVisible() && nextButton.isVisible()) {
			...
			app.getEngagementActionController().addContentToContentVBox(root, true);
			showPreviousAlert(
					"If new and/or different indicator cards are selected, your previously saved resilience and equity rankings will be reset.");
		} else if (previousButton.isVisible() && !nextButton.isVisible()) {
			quadrantSave();
			try {
				FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/noteboard/NoteBoardContent.fxml"));
				NoteBoard_LinearRanking noteBoardContentController = new NoteBoard_LinearRanking(app, goal,
						erbContentItem, new ArrayList<IndicatorCard>());
				fxmlLoader.setController(noteBoardContentController);
				VBox root = fxmlLoader.load();
